...
public class Character : MonoBehaviour, IEquipped
{
    ...
    */
    ...
    public void GearEquipped( ItemStats _stats, bool added )
    {
        ...
        {
            if ( added ) stats[StatID.Def_Phys_Flat] += _stats.Defense;
            else stats[StatID.Def_Phys_Flat] -= _stats.Defense;
        }

        foreach ( ItemStats.Prefix item in _stats.prefixes )
        {
            if ( added ) stats[(StatID)item.type] += item.value;
            else stats[(StatID)item.type] -= item.value;
        }
